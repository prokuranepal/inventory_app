...
import CategoryGridTile from './CategroyGridTile';
import {
    configure,
    shallow
} from 'enzyme';
...
describe('<CategoryGridTile />', () => {
  it("Events and props test in CategoryGridTile if deletable is false", () =>
        {
            const wrapper = shallow(<CategoryGridTile  {...dummy_data} />)
            ...
            const wrapper2 = shallow(<CategoryGridTile  {...{...dummy_data,iconValue:"drug-pack"}} />)
...
test_function(["<CategoryGridTile/>",1,1],['renders correctly','renders the length of view'],[<CategoryGridTile />,TouchableOpacity,Text],extra)
